    ...
    padding: 6px 30px;"  data-toggle="modal" data-target="#mediaGallery" >
              ...
              <div class="search-form-find">
               ...
               'controller' => 'tags','action' => 'searchTags'
                 )]);
                echo  $this->Form->control('name',array('label' => false,'class'=>'form-control','min'=>6));
                echo  $this->Form->end(); 
             ?>
               <p class="search-form-find-title">Tags</p>
            </div>
            </div>
              ...
              <table class="table table-bordered table-striped" id="example1">
                ...
                <?php   foreach ($tags as $tag) {
                  echo "<tr>
                  <td><input type='checkbox' value=".$tag->id." class='delete_item'></td>
                  <td>".$tag->id."</td>
                  <td>".$tag->name."</td>
                  
                 <td class='table__flex'>";
                 echo   $this->Html->link('<i class="fa fa-pencil"></i>', ['action' => 'edit', $tag->id], ['class'=>'btn change__user','escape' => false]);
                 echo  $this->Form->postLink('<i class="fa fa-trash"></i>', ['action' => 'delete', $tag->id], ['class'=>'btn  delete__user','escape' => false,'confirm' => __('Are you sure you want to delete # {0}?', $tag->id)]);  
                  echo "</td>";
                  echo "</tr>";
                } ?>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="gallery-box form__inline" style="text-align: center;">
          <h2 style="text-align: center;">Do you want to delete tags ?</h2>
          
                   <button class="close_modal_form close__modal" >No</button>
                   <?= $this->Form->create('Delete',['url'   => array(
               'controller' => 'tags','action' => 'deletechecked'
           )])  ?>
